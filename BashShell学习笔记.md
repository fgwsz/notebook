...
#### 1. `shopt -s globstar`
- **作用**：启用 **`**`** 通配符的**递归匹配**。
- 默认情况下，`*` 只匹配当前目录下的文件和目录，不会进入子目录。
- 开启 `globstar` 后，`**` 可以匹配**任意层级的子目录**。
- 例如：`export/**/*` 会匹配 `export/` 下的所有子目录中的任意文件和目录（包括深层嵌套）。
- **注意**：单独使用 `**` 本身可以匹配当前目录及所有子目录，但通常配合 `/*` 或其他模式来限定范围。
#### 2. `shopt -s nullglob`
- **作用**：当通配符**没有匹配到任何文件/目录**时，该通配符会被**展开为空字符串**（即被删除），而不是原样保留。
- 默认情况下（`nullglob` 未启用），如果一个通配符没有匹配项，它会保留为字面字符串。
- 例如，对于 `for file in dir/*; do`，如果 `dir/` 为空，则 `file` 会被赋值为 `"dir/*"` 这个字符串，可能导致后续判断出错。
- 开启 `nullglob` 后，如果 `dir/*` 没有匹配项，循环体根本不会执行（因为 `for` 的列表为空），这大大提高了脚本的健壮性。
#### 3. `shopt -s dotglob`
- **作用**：让 `*` 和 `**` 通配符**匹配以点（`.`）开头的隐藏文件**（如 `.bashrc`、`.hidden`）。
- 默认情况下，`*` 不会匹配隐藏文件（除了 `.` 和 `..` 始终被忽略）。
- 开启 `dotglob` 后，`*` 会包括隐藏文件，但**仍然会排除 `.` 和 `..` 目录**（避免递归死循环）。
#### 组合效果
```bash
shopt -s globstar nullglob dotglob
for file in "$TARGET_DIR"/**/*; do
```
- 可以**递归遍历** `$TARGET_DIR` 下的**所有子目录和文件**（包括隐藏文件）。
- 如果没有任何匹配项（比如目录为空），循环**不会执行**（避免处理不存在的模式）。
- 匹配项会包含以 `.` 开头的文件，如 `.hidden_file`。
