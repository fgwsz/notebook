...
**性能模式**和**平衡模式**的核心区别在于 **CPU 的频率调度策略(即"调速器",
Governor)** 和 **硬件响应速度**.

简单来说:**性能模式追求"时刻满血",平衡模式追求"随用随给".**

1. 核心机制:CPU 频率调度策略

这是两者最本质的差别:

-   **性能模式**:将 CPU 调速器强制锁定为 **`performance`**.
这意味着 CPU 会**始终运行在最高可用频率**上,即使系统完全空闲,也不会降频.
系统不会为了省电而让 CPU"休息".

-   **平衡模式**:将 CPU 调速器设置为 **`schedutil`** 或 **`ondemand`**
(取决于内核版本).这意味着 CPU 频率会**动态缩放**.
当你在打字或浏览静止页面时,频率降到最低以省电;
当你点击应用或编译代码时,频率在**几毫秒内**迅速拉升到最高以应对负载.

2. 实际体验上的差异(感知层面)

| 对比维度 | 🚀 性能模式 (Performance) | ⚖️ 平衡模式 (Balanced) |
| :--- | :--- | :--- |
| **响应速度** | **极致顺滑**.点击图标|切换窗口|打开大文件时几乎零延迟,因为 CPU 随时待命. | **略有延迟**.在点击重载任务(如打开 Chrome 或 IDE)时,会有极其细微的"等 CPU 提频"的过程(约 10-50ms),虽然不明显,但重度用户能感知. |
| **功耗与续航** | **耗电巨大**.笔记本电池续航可能缩短 **20%~30%**,因为空闲时也在全速耗电. | **省电明显**.空闲时大幅降频,是笔记本脱离电源时的首选,能显著延长使用时间. |
| **发热与风扇** | **温度较高**.CPU 持续发热,风扇会频繁高速转动,噪音较大. | **温度平稳**.大部分时间处于低温状态,风扇安静,只有在高负载瞬间才会短暂启动. |
| **跑分性能** | **基准测试分数最高**(如 Geekbench 多核分数可达上限). | **跑分略低**(约 5%~10% 差异),但在长时间持续高负载(如视频渲染)下,两者最终完成时间差距并不大,因为平衡模式在检测到持续负载后也会拉满频率. |

3. 背后的硬件限制(重要)

需要注意的是,这两个模式都受到 **功耗墙(TDP)** 和
 **温度墙(Thermal Throttling)** 的限制:

-   在**性能模式**下,CPU 会优先触及"温度墙".一旦温度过高(如 90°C+),
系统为了物理安全依然会强制降频(触发降频保护),此时性能模式的效果会大打折扣.
因此,**性能模式在散热良好的台式机上效果最明显,在轻薄本上则容易被发热抵消优势.**

-   在**平衡模式**下,系统会优先避开"温度墙",通过主动降频来保持凉爽.

4. 我应该怎么选?

-   **首选"平衡模式"**(日常 90% 的情况):插着电源写代码|看网页|看视频时,
平衡模式足够流畅,且能让电脑保持安静凉爽,对硬件寿命更友好.
-   **切换到"性能模式"**(特定重载场景):当你在做
 **3D 渲染|大型游戏|大规模代码编译(如全量构建 Android/Linux 内核)**,
或者感觉系统明显卡顿时,切到性能模式可以榨干最后一滴性能,加快任务完成速度.
...
`Snipaste`的截图快捷键默认是`F1`,
全屏截图选择是`Alt + A`,
将当前框选范围作为图像复制到剪切板是回车键`Enter`,
退出截图是`Esc`.
### 安装`QQ`
...
```
#### `obs`录制视频具体流程
点击最右下角的`[控件]`面板里的`[开始录制]`按钮开始录制.

点击最右下角的`[控件]`面板里的`[结束录制]`按钮结束录制.
#### 设置`obs`录制视频快捷键
首先需要保证`obs`是打开的状态.
点击最右下角的`[控件]`面板里的`[设置]`按钮进入设置页.
```txt
...
```
此时会打开一个图形界面.
单击`Data project`,点击`+`号添加需要刻录到光盘之中的文件.

选择要写入到光盘之中的文件,然后点击右下角的`Burn`按钮,即可进入下一步设置.

在点击`Burn`按钮之前可能会出现一些情况,导致无法选中光盘的盘符,
而是新建一个`brasero.iso`文件,而非写入光盘之中.

出现这种情况的根源在于有一些光盘是`CD/DVD+R`(支持多次刻录的是`CD/DVD+RW`),
它们只能刻录一次,已经刻录过之后,就无法再次刻录写入数据了,只能读取文件.

因此,当出现这种情况时,`brasero`会自动放弃写入光盘,转为新建一个`.iso`文件.

如果没有出现上述情况,接下来就是`Burn`设置页.

如果是只写入一张光盘的话,请直接点击右下角的`Burn`按钮即可.
